...
void solution::print(const problem& input) const {
	cout << "[" << routes.size() << " routes, distance = " << maxDistance << ", timewarp = " << totalTimewarp << ", unbalance = " << unbalancedCapacity << " " << feasible << "]" << endl;
...
void solution::init(int maxRoutes, const problem& input, vector<vector<int>> tours){
	...
	for(int i = 0; i < maxRoutes; ++i){
		if(newRoutes[i].size() > 0){
			...
			newRoute.modified = true;
//			routes.push_back(newRoute);
			routes.insert(routes.end(), newRoute);
		}
	}
...
int solution::cmp(const solution &solA, const solution &solB, const problem &input){
	if(solA.feasible != solB.feasible){
		...
		if(solA.maxDistance != solB.maxDistance ) return (solA.maxDistance - solB.maxDistance );
		else if(solA.totalDistance < solB.totalDistance) return -1;
...
bool solution::fdominate(const solution &solA, const solution &solB){
	...
	if( solA.totalDistance <= solB.totalDistance && solA.maxDistance <= solB.maxDistance ){
		return ( solA.totalDistance < solB.totalDistance || solA.maxDistance < solB.maxDistance );
	}else return false;
