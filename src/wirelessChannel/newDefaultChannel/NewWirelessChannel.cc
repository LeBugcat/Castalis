...
#include "wirelessChannel/newDefaultChannel/NewWirelessChannel.h"
//#include "wirelessChannel/defaultChannel/WirelessChannel.h"

Define_Module(NewWirelessChannel);

int NewWirelessChannel::numInitStages() const
{
	return 2;
}

double NewWirelessChannel::computePathloss(double distance, double frequency, double treeDepth){
    switch(pathlossModel)
    {
        case LNS:
            return PLd0 + 10.0 * pathLossExponent * log10(distance / d0) + normal(0, sigma);
        case MED:
            if(treeDepth<14){
                return 0.45 * pow(frequency, 0.284) * distance;
            } else return 1.33 * pow(frequency, 0.284) * pow(distance, 0.588);
        case ITU_R:
            if(frequency>3e+8 && frequency<3e+9){
                return 0.2 * pow(frequency, 0.3) * pow(distance,0.6);
            } else {
                throw cRuntimeError("ERROR: ITU_R model only works for UHF frequencies");
                break;
            };
        default:
            return PLd0 + 10.0 * pathLossExponent * log10(distance / d0) + normal(0, sigma);
...
void NewWirelessChannel::initialize(int stage){
...
//				PLd = PLd0 + 10.0 * pathLossExponent * log10(dist / d0) + normal(0, sigma);
			    PLd = computePathloss(dist, expectedFreq, treeDepth);
 ...
 *****************************************************************************/
void NewWirelessChannel::handleMessage(cMessage * msg)
{
	switch (msg->getKind()) {

	case WC_NODE_MOVEMENT:{
	        applicationTrace() << "node movement asked from Node ";
...
//			if(not neighborTable[srcAddr] or neighborTable[srcAddr].empty()){
//
//			}
//            neighborTable[srcAddr] = nodesAffectedByTransmitter->c;
...
void NewWirelessChannel::finishSpecific()
{
	...
	DebugInfoWriter::closeAppStream();
}

void NewWirelessChannel::readIniFileParameters(void)
{
	DebugInfoWriter::setDebugFileName(
		getParentModule()->par("debugInfoFileName").stringValue());
    DebugInfoWriter::setAppDebugFileName(
        getParentModule()->par("debugAppFileName").stringValue());
    DebugInfoWriter::setMetricsDebugFileName(
        getParentModule()->par("debugMetricsFileName").stringValue());
 ...
 */
void NewWirelessChannel::parsePathLossMap(void)
{
...
void NewWirelessChannel::updatePathLossElement(int src, int dst, float pathloss_db)
{
...
int NewWirelessChannel::parseInt(const char *c, int *dst)
{
	while (c[0] && (c[0] == ' ' || c[0] == '\t'))
		c++;
	if (!c[0] || c[0] < '0' || c[0] > '9')
...
int NewWirelessChannel::parseFloat(const char *c, float *dst)
{
	char *tmp;
	*dst = strtof(c, &tmp);
	if (c == tmp)
