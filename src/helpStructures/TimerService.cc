...
void TimerService::setTimer(int timerIndex, simtime_t time) //set timer action in seconds
{
	if (timerIndex < 0)
...
void TimerService::setTimer(int timerIndex, simtime_t time, std::string message) //set timer action in seconds
{
    if (timerIndex < 0)
        throw cRuntimeError("setTimer(): timerIndex=%i negative index is not allowed",timerIndex);
    if (timerIndex >= TIMER_MAX_SIZE)
    ...
    timerMessages[timerIndex] = new TimerServiceMessage(message.c_str(), TIMER_SERVICE);
    timerMessages[timerIndex]->setTimerIndex(timerIndex);
    scheduleAt(simTime() + timerDrift * time, timerMessages[timerIndex]);
}
