...
// check if a Point is inside (including lies on edge) of a grid
    ...
    bool G::isSegmentInsidePolygon(Point s, Point d, std::vector<Point> p) {
        if (G::segmentPolygonIntersect(s, d, p))
            ...
            if (!G::isPointReallyInsidePolygon(G::midpoint(s, d), p)) // todone: cung chua chuan lam nhung tam on trong hau het cac truong hop can dung
                return false;                                   // todone: cung co the midpoint van la dinh cua polygon
            else return true;
        }
    }
    ...
    bool G::isSegmentInsidePolygon1(Point s, Point d, std::vector<Point> p) {
        if (G::segmentPolygonIntersect1(s, d, p))
            ...
            if (!G::isPointInsidePolygon(G::midpoint(s, d), p)) // todone: cung chua chuan lam nhung tam on trong hau het cac truong hop can dung
                return false;                                   // todone: cung co the midpoint van la dinh cua polygon
            else return true;
        }
    }
