...
bool GlobalLocationService::rangeUpdated = false;

double GlobalLocationService::cellWidth = 1;
double GlobalLocationService::cellHeight = 1;
double GlobalLocationService::range=0;
...
void GlobalLocationService::initialize(cModule *module) {
  ...
  if(!rangeUpdated)
    throw cRuntimeError("range not updated, false logic, abort simulation");

  if (!initialized) {
    networkModule = module;
    initialized = true;

    int numNodes = (int) module->par("numNodes");
...
std::vector<int> GlobalLocationService::getNeighborIndex(int id) {
  vector<int> res;
  for(int i=0; i<neighborTables[id].size(); i++){
      res.push_back(neighborTables[id][i].id);
  }
  return res;
}
