...
vector<int> VirtualApplication::nodesDeathTimestamp;
vector<int> VirtualApplication::isAlive;
...
void VirtualApplication::handleMessage(cMessage * msg)
{
	...
	switch (msgKind) {
		...
		case APPLICATION_PACKET:
		{
			...
			// application_trace()<<"PACKET ARRIVED AT LOWER VIRTUAL APP";
...
void VirtualApplication::finish()
{
    ...
    DebugInfoWriter::closeMetricsStream();
}
...
//		throw cRuntimeError("toNetworkLayer() function used incorrectly to send APPLICATION_PACKET without destination Network address");
    //bypass checking, this is a very bad design
//    routing_trace()<<"SENDING FROM APP";
...
//		size << " bytes to communication layer";
	...
	// application_trace() << "REPORTING: packet length at App, passing on to " << appPkt->getByteLength() << "bytes.";
