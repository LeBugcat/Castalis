...
//                    currentFrameStart += new_offset;
				    ...
				    //ATTENTION: THIS IS ONLY A WEIRD MECHANISM TO ENSURE
				    //THAT THIS TIMER SIGNAL NEVER IS NEGATIVE
				    if(wakeup<0) {
				        macTrace() << "WARN::BAD mechanism used, check config logic";
				        setTimer(FRAME_START, 0);
				    }
				    else setTimer(FRAME_START, wakeup);
					currentFrameStart += new_offset;
				} else {
...
//			if(waitingOnData) break; //early stopping to ensure no double CTS is issued
...
//			// update MAC state
//			waitingOnData = true; //now we are waiting for the data pkt
