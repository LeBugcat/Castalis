		...
		case PERFORM_CCA: {
			...
			if (CCAcode == CLEAR) {
				...
				if (++NB > macMaxCSMABackoffs) {
					...
					attemptTransmission("Current NB exceeded maxCSMAbackoffs");
				} else {
					performCSMACA();
				}
...
void Basic802154::fromRadioLayer(cPacket * pkt, double rssi, double lqi)
{
	...
	switch (rcvPacket->getMac802154PacketType()) {
		...
		case MAC_802154_BEACON_PACKET: {
			...
			//this is how the authors calculate timeee
			CAPend = CAPlength * baseSlotDuration * (1 << frameOrder) * symbolLen;
			GTSstart = 0;
			GTSend = 0;
			GTSlength = 0;
...
void Basic802154::attemptTransmission(const char * descr)
{
	...
	// if a packet already queued for transmission
	// - check available retries and delay
	// this means that there is only ONE single current packet
	// queuing up for transmission, at all times
