...
enum MacStates {
	...
	MAC_STATE_CAP = 1002,
	MAC_STATE_CFP = 1004, //comprise of many GTSs and SLEEPs
};
...
class Basic802154: public VirtualMac {
 ...
 protected:
	...
	map<string,int> packetBreak; //equal to a Python dict
	...
	int macBSN;			// beacon sequence number (unused, prob used in round protocols)
	int lostBeacons;	// number of consequitive lost beacon packets

	//TODOne: figure out what do "symbols" mean?
	...
	int NB, CW, BE;
	// CSMA-CA algorithm parameters (
	//  Number of Backoffs, Contention Window length
	//  and Backoff Exponent)

	//packet variables.
	//Note that the MAC protocol handles 1 packet at a time
	...
	bool isPANCoordinator; //equal to Sink in star nets
	bool isFFD;//Fully Functional Device. todone: how is this diff from normal nodes?
	//this is a fucking scam, isFFD === isPAN! todone: clean up this FFD bs, every node is FFD now
	...
	//not set in ned and is infered from frameslot unit equivalences
	...
	int associatedPAN;	// ID of current PAN (-1 if not associated to any PANs)
...
//  protected:
	...
	//direct call to destination are implemented down at the Virtual level
//    void fromMacLayerDirect(cPacket *);
//    void toMacLayerDirect(cPacket *, int);

	//THESE MUST BE IMPLEMENTED,
	//TODONE: see to it that most of these is covered
	//      in the TSCH extended class
