...
class Radio: public CastaliaModule {
 ...
 public:
    ...
    double datarate;
//    string radioParamsFilePath;

    PktBreakdown stats;
    PktBreakdown currentRoundStats;
 ...
 public:
	...
	int getCollisionModel(){
	   return (int) collisionModel;
	}
	void setCollisionModel(int type){
	    this->collisionModel = static_cast<CollisionModel_type>(type);
	}

	double readRSSI();
	CCA_result isChannelClear();

	void printRoundStats(int roundId){
	    metricsTrace()<< "Collected metrics";
        metricsTrace() << "Round " << roundId
                << " " << currentRoundStats.transmissions
                << " " << currentRoundStats.RxReachedNoInterference // packets reached
                << " " << currentRoundStats.RxReachedInterference  // packets reached despite interference
                << " " << currentRoundStats.RxFailedNoInterference // packets failed even without interference
                << " " << currentRoundStats.RxFailedInterference;   // packets failed with interference
//        metrics_trace() << "RxFailedModulation "<<currentRoundStats.RxFailedModulation;     // packets failed, wrong modulation
    }

    void resetRoundStats(){
        currentRoundStats.transmissions=0;
        currentRoundStats.RxReachedNoInterference=0;// packets reached
        currentRoundStats.RxReachedInterference=0;  // packets reached despite interference
        currentRoundStats.RxFailedNoInterference=0; // packets failed even without interference
        currentRoundStats.RxFailedInterference=0;   // packets failed with interference
        currentRoundStats.RxFailedModulation=0;     // packets failed, wrong modulation
        currentRoundStats.RxFailedNoRxState=0;      // packets failed, radio not in RX
        currentRoundStats.bufferOverflow=0;         // packets overflown
    }
