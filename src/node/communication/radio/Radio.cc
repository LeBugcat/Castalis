...
void Radio::handleMessage(cMessage * msg)
{
	...
	switch (msg->getKind()) {
		...
		case WC_SIGNAL_START:{
			...
			/* If we are in RX state, go through the list of received signals and update
		...
		case WC_SIGNAL_END:{
			...
			if (endingSignal->bitErrors != ALL_ERRORS) {
				if (endingSignal->bitErrors <= maxErrorsAllowed(endingSignal->encoding)) {
					// decapsulate the zz the RSSI and LQI fields
...
//				// application_trace()<<"POWER DRAWN called";
		...
		case MAC_LAYER_PACKET:{
			...
			if (maxPhyFrameSize != 0 && totalSize > maxPhyFrameSize) {
				...
				throw cRuntimeError("packet too big, expand the PHY maxFrameSize");
				break;
			}
 ...
 * been set for the current radioState change.
 */
...
double Radio::popAndSendToWirelessChannel()
{
    ...
    // application_trace() << "Finally SENDING FROM RADIO packet w SIZE " << end->getByteLength();

	//calculate the TX time based on the length of the packet
	//  double txTime = ((double)(end->getByteLength() * 8.0f)) / RXmode->datarate;
	double txTime = ((double)(end->getByteLength() * 8.0f)) / RXmode->datarate;

	// application_trace() << "REPORTING: TxTime" << txTime << "at Radio layer";

	//employ this instead of timer service, since this is the only time-related function
					...
					WirelessChannelSignalBegin * wcMsg)
{
	switch (collisionModel) {
		...
		case SIMPLE_COLLISION_MODEL:{
			// an arbitrary rule: if the signal is more than 6dB less than sensitivity,
			// interference is considered catastrophic.
...
//	radioParamsFilePath = par("RadioParametersFile");
...
void Radio::parseRadioParameterFile(const char *fileName)
{
	...
	//note that we can do this only because the datarate is fixed
	// in all of the Modes of these radios
	std::string fileName_str = fileName;

	if(fileName_str == "../Parameters/Radio/CC2420.txt"
	        or fileName_str == "../Parameters/Radio/NOTIMESTATECHANGE.txt")
	{
        datarate = 250000.0;// bit/s
    } else if(fileName_str == "../Parameters/Radio/CC1000.txt"){
        datarate = 19200.0;// bit/s
    } else throw cRuntimeError("Unknown Radio using, can't deduce datarate");
